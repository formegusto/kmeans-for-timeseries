...
class KMeans:
    ...
    # type is 'euclidean' or 'cosine'
    def run(self, K=10,  alg_type="euclidean"):
        if self.logging:
        ...
        self.K = math.ceil(math.sqrt((len(self.datas.columns) / 2)))
        K = self.K
        ...
        while True:
            ...
            cols = ['value']
            # asc = [True, False]
            asc = [True] if alg_type == 'euclidean' else [False]
            alg = utils.euclidean_distance if alg_type == "euclidean" else utils.cosine_similarity
